...
for(i in sessions){
  ...
  # dir.create(paste0("data/models/5lemma_trigram/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/randomforest_estimations/preds/5lemma_trigram/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/randomforest_estimations/prob_grids/5lemma_trigram/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  
  done_folds <- list.files(paste0("data/randomforest_estimations/preds/5lemma_trigram/", gsub("data/folds/", "", i)))
  ...
  tmp_trigram <- tmp_anot %>% 
    ...
    ungroup() %>%                                                                         # Ungrouping
    count(id, lemma, lemma_next, lemma_nextnext) %>%                                      # Counting tokens
    tidyr::unite(lemma, lemma, lemma_next, lemma_nextnext, sep = "_")                     # Uniting bigram
  ...
  tmp_bigram <- tmp_anot %>% 
    ...
    filter(str_detect(lemma_next, "\\s") == FALSE) %>%                           # Cleaning som noise
    ungroup() %>%                                                                # Ungrouping
    count(id, lemma, lemma_next) %>%                                             # Counting tokens
    tidyr::unite(lemma, lemma, lemma_next, sep = ":")                            # Uniting bigram
  ...
  preds <- lapply(1:length(train), function(x){
    ...
    write_csv(pred, path = paste0("data/randomforest_estimations/preds/5lemma_trigram/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(prob_grid, path = paste0("data/randomforest_estimations/prob_grids/5lemma_trigram/", gsub("data/folds/", "", i), "/", fold, ".csv"))
