...
for(i in sessions){
  ...
  dir.create(paste0("data/preds/0baseline/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/prob_grids/0baseline/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  ...
  preds <- lapply(1:length(train), function(x){
    ...
    # Making sure folders are in place
    dir.create(paste0("data/preds/0baseline/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
    dir.create(paste0("data/prob_grids/0baseline/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
    # dir.create(paste0("data/models/0baseline/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
    ...
    write_csv(pred, path = paste0("data/preds/0baseline/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(prob_grid, path = paste0("data/prob_grids/0baseline/", gsub("data/folds/", "", i), "/", fold, ".csv"))
