...
# Tuning control for nn (no tuning)
tr <- trainControl(method = "none",
                   classProbs = TRUE,
                   verboseIter = TRUE) 

# Tuning grid for nn (on average best performing tuning grid)
tg <- expand.grid(size = 10)
...
for(i in sessions){
  ...
  # dir.create(paste0("data/models/6lemma_pos_trigram/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/nn_estimations/preds/6lemma_pos_trigram/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/nn_estimations/prob_grids/6lemma_pos_trigram/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  
  done_folds <- list.files(paste0("data/nn_estimations/preds/6lemma_pos_trigram/", gsub("data/folds/", "", i)))
  ...
  rm(tmp_trigram, tmp_bigram, tmp_unigram, tmp_anot)
  ...
  preds <- lapply(1:length(train), function(x){
    ...
    write_csv(pred, path = paste0("data/nn_estimations/preds/6lemma_pos_trigram/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(prob_grid, path = paste0("data/nn_estimations/prob_grids/6lemma_pos_trigram/", gsub("data/folds/", "", i), "/", fold, ".csv"))
