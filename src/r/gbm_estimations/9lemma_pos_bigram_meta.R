...
for(i in sessions){
  ...
  # dir.create(paste0("data/models/9lemma_pos_bigram_meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/preds/9lemma_pos_bigram_meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/prob_grids/9lemma_pos_bigram_meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/var_importance/9lemma_pos_bigram_meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  
  done_folds <- list.files(paste0("data/gbm_estimations/preds/9lemma_pos_bigram_meta/", gsub("data/folds/", "", i)))
  ...
  tmp_dfm <- bind_rows(tmp_unigram, tmp_bigram) %>%
    arrange(id, n) %>% 
  ...
  rm(tmp_bigram, tmp_unigram, tmp_anot, meta_vars)
  ...
  preds <- lapply(1:length(train), function(x){
    ...
    write_csv(pred, path = paste0("data/gbm_estimations/preds/9lemma_pos_bigram_meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(prob_grid, path = paste0("data/gbm_estimations/prob_grids/9lemma_pos_bigram_meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(imp, path = paste0("data/gbm_estimations/var_importance/9lemma_pos_bigram_meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
