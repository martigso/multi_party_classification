...
for(i in sessions){
  ...
  # dir.create(paste0("data/models/88meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/preds/88meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/prob_grids/88meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/var_importance/88meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  
  done_folds <- list.files(paste0("data/gbm_estimations/preds/88meta/", gsub("data/folds/", "", i)))
  ...
  tmp_dfm <- tmp_meta[, c("id", meta_vars)] %>% 
    rename(document = id) %>% 
  ...
  tmp_dfm <- data.frame(tmp_dfm, stringsAsFactors = FALSE)
  #tmp_dfm[1:10, 1:7]
  ...
  preds <- lapply(1:length(train), function(x){
    ...
    write_csv(pred, path = paste0("data/gbm_estimations/preds/88meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(prob_grid, path = paste0("data/gbm_estimations/prob_grids/88meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(imp, path = paste0("data/gbm_estimations/var_importance/88meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
  })
