...
for(i in sessions){
  ...
  dir.create(paste0("data/gbm_estimations/preds/1lemma/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/prob_grids/1lemma/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  
  done_folds <- list.files(paste0("data/gbm_estimations/preds/1lemma/", gsub("data/folds/", "", i)))
  ...
  preds <- lapply(1:length(train), function(x){
    ...
    write_csv(pred, path = paste0("data/gbm_estimations/preds/1lemma/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(prob_grid, path = paste0("data/gbm_estimations/prob_grids/1lemma/", gsub("data/folds/", "", i), "/", fold, ".csv"))
