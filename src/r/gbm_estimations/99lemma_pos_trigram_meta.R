...
suppressMessages(library(gbm))          # Gradient boost extra utilities
...
# Making MP age
meta$rep_age <- as.numeric((meta$date - as.Date(meta$rep_birth, "%d.%m.%Y")) / 365.25)
...
for(i in sessions){
  ...
  # dir.create(paste0("data/models/99lemma_pos_trigram_meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/preds/99lemma_pos_trigram_meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/prob_grids/99lemma_pos_trigram_meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  dir.create(paste0("data/gbm_estimations/var_importance/99lemma_pos_trigram_meta/", gsub("data/folds/", "", i)), recursive = TRUE, showWarnings = FALSE)
  
  done_folds <- list.files(paste0("data/gbm_estimations/preds/99lemma_pos_trigram_meta/", gsub("data/folds/", "", i)))
  ...
  # Trigram pre-processing
  tmp_trigram <- tmp_anot %>% 
    mutate(lemma_pos = paste(lemma, part_of_speech, sep = "_")) %>% 
    select(id, sentence, lemma, lemma_pos) %>% 
    group_by(id, sentence) %>% 
    mutate(lemma_next = lead(lemma),
           lemma_pos_next = lead(lemma_pos),
           lemma_nextnext = lead(lemma_next),
           lemma_pos_nextnext = lead(lemma_pos_next)) %>% 
    filter(str_detect(lemma, "[[:punct:]]") == FALSE) %>%                                 # Removing punctuation
    filter(str_detect(lemma_next, "[[:punct:]]") == FALSE) %>%                            # Removing punctuation
    filter(str_detect(lemma_nextnext, "[[:punct:]]") == FALSE) %>%                        # Removing punctuation
    filter(str_detect(lemma, "[0-9]+") == FALSE) %>%                                      # Removing numbers
    filter(str_detect(lemma_next, "[0-9]+") == FALSE) %>%                                 # Removing numbers
    filter(str_detect(lemma_nextnext, "[0-9]+") == FALSE) %>%                             # Removing numbers
    filter(lemma %in% stwords == FALSE) %>%                                               # Removing stopwords
    filter(lemma_next %in% stwords == FALSE) %>%                                          # Removing stopwords
    filter(lemma_nextnext %in% stwords == FALSE) %>%                                      # Removing stopwords
    filter(lemma != "" & str_detect(lemma, "^\\s+$") == FALSE)  %>%                       # Cleaning som noise
    filter(lemma_next != "" & str_detect(lemma_next, "^\\s+$") == FALSE)  %>%             # Cleaning som noise
    filter(lemma_nextnext != "" & str_detect(lemma_next, "^\\s+$") == FALSE)  %>%         # Cleaning som noise
    filter(str_detect(lemma, "\\s") == FALSE) %>%                                         # Cleaning som noise
    filter(str_detect(lemma_next, "\\s") == FALSE) %>%                                    # Cleaning som noise
    filter(str_detect(lemma_nextnext, "\\s") == FALSE) %>%                                # Cleaning som noise
    ungroup() %>%                                                                         # Ungrouping
    count(id, lemma_pos, lemma_pos_next, lemma_pos_nextnext) %>%                          # Counting tokens
    tidyr::unite(lemma_pos, lemma_pos, lemma_pos_next, lemma_pos_nextnext, sep = "_")     # Uniting bigram
  
  # Bigram pre-processing
  tmp_bigram <- tmp_anot %>% 
    mutate(lemma_pos = paste(lemma, part_of_speech, sep = "_")) %>% 
    select(id, sentence, lemma, lemma_pos) %>% 
    group_by(id, sentence) %>% 
    ...
    filter(str_detect(lemma_next, "\\s") == FALSE) %>%                           # Cleaning som noise
    ungroup() %>%                                                                # Ungrouping
    count(id, lemma_pos, lemma_pos_next) %>%                                     # Counting tokens
    tidyr::unite(lemma_pos, lemma_pos, lemma_pos_next, sep = ":")                # Uniting bigram
  ...
  tmp_dfm <- bind_rows(tmp_unigram, tmp_bigram, tmp_trigram) %>%
    arrange(id, n) %>% 
  ...
  # Meta variables
  meta_vars <- c("rep_gender", "rep_age", "county", "debate_type", "case_type", "language")
  ...
  rm(tmp_trigram, tmp_bigram, tmp_unigram, tmp_anot, meta_vars)
  ...
  preds <- lapply(1:length(train), function(x){
    ...
    write_csv(pred, path = paste0("data/gbm_estimations/preds/99lemma_pos_trigram_meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(prob_grid, path = paste0("data/gbm_estimations/prob_grids/99lemma_pos_trigram_meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
    write_csv(imp, path = paste0("data/gbm_estimations/var_importance/99lemma_pos_trigram_meta/", gsub("data/folds/", "", i), "/", fold, ".csv"))
